...
function Layout() {
  ...
  const navLinks = [
    ...
    { path: '/methode', label: t('nav.methode') },
    { path: '/a-propos', label: t('nav.aPropos') },
  ];
    ...
    <>
      ...
      <header className="fixed top-0 z-[100] w-full bg-surface/90 backdrop-blur-md border-b-[0.5px] border-outline-variant/30 flex justify-between items-center px-6 md:px-12 py-6">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <button 
            onClick={toggleTheme}
            className="w-10 h-10 flex items-center justify-center rounded-full border border-outline-variant/30 text-on-surface hover:bg-on-surface/5 transition-all duration-300"
      ...
      <div className={`fixed inset-0 z-[90] bg-surface transition-all duration-500 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full justify-center px-12 gap-8">
          ...
          >
            {t('nav.launch')} <span className="text-2xl">→</span>
          </Link>
        </div>
      ...
      <footer id="main-footer" className="bg-surface border-t border-outline-variant/30 flex flex-col md:flex-row justify-between items-center px-6 md:px-12 py-12 w-full gap-8">
        ...
        <div className="flex flex-wrap justify-center gap-8">
            ...
            <Link className="font-space text-[10px] tracking-[0.2em] uppercase text-on-surface-variant hover:text-primary transition-all duration-100" to="/mentions-legales">{t('footer.legal')}</Link>
        </div>
      </footer>
    </>
  );
